...
| Internet orchestration concurrency | `EgcfStore` holds one exclusive workspace lock for the lifetime of each Director/Orchestrator invocation, including provider calls. | The qualified deployment model is a system timer invoking bounded `run-once` processes sequentially; a continuously running multi-process daemon is not claimed. |
| Internet transport idempotency | Durable action keys, leases, and receipts prevent repeated StateWright lifecycle transitions, but an HTTP server can observe a request before a local crash records the receipt. | Fetches remain policy-bounded and content-addressed; the release claims restart-safe reconciliation, not exactly-once external transport. |
